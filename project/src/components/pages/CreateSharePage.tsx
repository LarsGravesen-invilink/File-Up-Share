...
import { generateId, formatBytes } from '../../helpers';
...
export function CreateSharePage({ settings, onAdd }: Props) {
  ...
  const publish = async () => {
    ...
    const id = generateId();
    const now = Date.now();
    const ms = (lifetimeHours * 3600000) + (lifetimeMinutes * 60000);
  ...
  const demoShare = {
    ...
    hideTimer,
    password: settings.sharePasswordEnabled ? settings.sharePassword : (usePassword ? password : ''),
    expiresAt: Date.now() + (lifetimeHours * 3600000) + (lifetimeMinutes * 60000),
  };
            ...
            <div className="flex items-center gap-2">
              <div className="flex-1">
                <input
                  type="number"
                  value={lifetimeHours}
                  onChange={e => setLifetimeHours(Math.max(0, Number(e.target.value)))}
              ...
              <div className="flex-1">
                <input
                  ...
                  onChange={e => setLifetimeMinutes(Math.min(59, Math.max(0, Number(e.target.value))))}
                  onFocus={e => e.currentTarget.select()}
                  min={0}
                  max={59}
                  className="w-full rounded-lg border border-white/8 bg-white/5 px-3 py-2 text-sm text-white outline-none focus:border-cyan-500/30"
