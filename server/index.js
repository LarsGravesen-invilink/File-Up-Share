...
const writeJSON = (name, data) => {
  try {
    ...
    console.error('Write error:', name, e.message);
  }
};
...
const getStats = () => {
  try {
    ...
    try {
      ...
      totalDisk = parseInt(execSync('df -m / --output=avail 2>/dev/null | tail -1').toString().trim()) || 51200;
    } catch {}
    return {
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    if (url === '/api/state' && method === 'GET') {
      return json(res, {
        auth: { firstRun: !auth.passHash, loggedIn: !!session },
        settings, stats: getStats(), shares, uploads, received,
        stealth: fs.existsSync(fp('stealth.lock'))
      });
    }

    if (url === '/api/register' && method === 'POST') {
      const b = await parseBody(req);
      if (!b.user || !b.pass) return json(res, { ok: false, error: 'Пустые данные' });
    ...
    if (url === '/api/login' && method === 'POST') {
      if (Date.now() < lockUntil) {
        return json(res, { ok: false, locked: true, sec: Math.ceil((lockUntil - Date.now()) / 1000) });
      }
      const b = await parseBody(req);
      if (String(b.user) === auth.user && hash(b.pass) === auth.passHash) {
        session = genId();
        fails = 0;
        return json(res, { ok: true });
      }
      ...
      if (fails >= 3) {
        ...
        fails = 0;
        return json(res, { ok: false, locked: true, sec: 300 });
      }
    ...
    if (url === '/api/credentials' && method === 'POST') {
      const b = await parseBody(req);
      if (b.login) auth.user = String(b.login);
    ...
    if (url === '/api/shares' && method === 'POST') {
      ...
      return json(res, { shares });
    }

    if (url.match(/^\/api\/shares\//) && method === 'DELETE') {
    ...
    if (url.match(/^\/api\/received\//) && method === 'DELETE') {
      const rid = url.split('/').pop();
      received = received.filter(r => r.id !== rid);
      writeJSON('received.json', received);
      return json(res, { received });
    }

    if (url === '/api/stealth' && method === 'POST') {
      const b = await parseBody(req);
      try {
        if (b.on) fs.writeFileSync(fp('stealth.lock'), '1');
        else fs.unlinkSync(fp('stealth.lock'));
      } catch {}
      return json(res, { ok: true });
    }
...
server.listen(PORT, '0.0.0.0', () => {
  console.log('FileUpShare API on port ' + PORT);
});
