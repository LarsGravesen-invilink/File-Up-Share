...
import { Shield, Eye, EyeOff, KeyRound, ShieldAlert, Ghost, AlertTriangle, CheckCircle, Bot } from 'lucide-react';
  ...
  return (
    <div className="space-y-5">
      ...
      >
        ...
        {/* autocomplete="new-password" отключает автосохранение браузером */}
        <form autoComplete="off" onSubmit={e => e.preventDefault()}>
          <input type="text" name="username_fake" className="hidden" readOnly tabIndex={-1} />
      ...
      >
            ...
            <div className="mb-3 rounded-lg bg-white/3 px-4 py-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2 text-xs text-white/50">
                  ...
                  Бот слушает /hide и /show
                </div>
                <Toggle
                  checked={settings.botNotifyService ?? true}
                  onChange={v => onUpdate({ botNotifyService: v })}
                />
              </div>
              <p className="mt-1.5 text-[10px] text-white/15">
                Когда выключено — бот игнорирует команды /hide и /show
              </p>
            </div>
          </>
        )}
        ...
        {!(settings.botEnabled && settings.botToken) && (
          <div className="mb-3 rounded-lg bg-yellow-500/5 px-4 py-3">
            <p className="text-[11px] text-yellow-400/60">
              Для управления через бота настройте Telegram бота в разделе «Telegram бот».
            </p>
          </div>
        )}
